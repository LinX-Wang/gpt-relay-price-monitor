...
def render_quality_sites(items: list[dict[str, Any]]) -> str:
    ...
    for item in items:
        ...
        cards.append(
            f"<article class=\"quality-card\">"
            f"<div class=\"quality-name\">{name}</div>"
            f"<div class=\"quality-note\">{notes}</div>"
            f"<div class=\"quality-url\">{url}</div>"
            f"<a class=\"open-link\" href=\"{url}\" target=\"_blank\" rel=\"noopener noreferrer\">打开</a>"
            f"</article>"
        )
    return (
        "<section id=\"quality-sites\" class=\"quality-panel\">"
        "<div class=\"panel-bar\"><div class=\"panel-title\">验纯网站</div>"
        "<div class=\"quality-tip\">建议用低额度临时 key 测试</div></div>"
...
def export_html(snapshots: list[SiteSnapshot]) -> Path:
    ...
    quality_section = render_quality_sites(quality_sites)
    ...
    <section class="tool-panel">
      ...
      <div class="tool-content">
        <div>
          ...
          <div class="tool-desc">用于查看公开中转站目录、模型覆盖、最低倍率、公告和站点状态。适合作为你手动维护本站表格时的外部参考。</div>
        </div>
